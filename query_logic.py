        ...
        out.append(
            {
                ...
                # Full original dataset row (all columns), when available.
                "row": row,
            }
        )
    return out
...
def is_found_in_dataset(query: str, retrieved: list[dict[str, Any]]) -> bool:
    # After deterministic filtering, "found" should mean "we have at least one matching row".
...
def _extract_country_from_query(query: str) -> str | None:
    ...
    # If pycountry is available, detect any country name in the query.
    ...
    # Fallback: look for patterns like "in Japan" / "in Pakistan".
...
def _canonicalize_country(country: str, store_rows: list[dict[str, Any]]) -> str:
    # Map detected country text to the dataset's canonical country strings when possible.
    ...
    # Partial (e.g., "united states" -> "united states of america")
    for c in sorted(countries, key=len, reverse=True):
        if detected and detected in c:
            return c

    # If not in dataset, keep as-is so filtering yields Not found.
    return detected
...
def _term_in_haystack(term: str, haystack: str) -> bool:
    ...
    if not t:
        return False
    # Short terms (ai, us) must match whole word.
...
def _record_matches_constraints(
    ...
    *,
    ...
    if sector_groups:
        ...
        # Require at least one synonym match per group.
        for group in sector_groups:
            if not any(any(_term_in_haystack(t, h) for h in haystacks) for t in group):
                return False
...
def filter_retrieved(
    ...
    *,
    ...
    try:
        for m in getattr(store, "metas", []) or []:
            row = m.get("row")
    ...
    location = _extract_location_constraint(query, store_rows) if store_rows else None
